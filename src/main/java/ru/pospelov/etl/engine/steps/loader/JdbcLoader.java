...
public class JdbcLoader implements Loader {
    ...
    @Override
    public void load(Collection<Record> records, EtlJob job) {
        ...
        log.info("Loading {} records into {} in batches of {}", recordList.size(), targetTable, batchSize);

        for (int from = 0; from < recordList.size(); from += batchSize) {
            int to = Math.min(from + batchSize, recordList.size());
            List<Record> batch = recordList.subList(from, to);

            jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
                @Override
                public void setValues(PreparedStatement ps, int i) throws SQLException {
                    Record rec = batch.get(i);
                    int index = 1;
